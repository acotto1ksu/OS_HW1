...
int main() {
    ...
    for (int i = 0; i < PROD_ITERATIONS; i++) {
        sem_wait(sem_can_produce); //Wait until able to produce one more
        std::cout << getpid() << ": Entered critical section. // ";
        sem_wait(sem_mutex); //Enter critical section

        data->buffer[data->in] = table_counter++; //Add item to table
        std::cout << "Produced " << table_counter-1 << " at " << data->in << ".";
        data->in = (data->in + 1) % TABLE_SIZE; //Move in position
    
        sem_post(sem_mutex); //Exit critical section
        std::cout << " // Exited critical section.\n" << std::endl;
        sem_post(sem_can_consume); //Signal consumer to consume one more
